...
- `src/pages/Photos.tsx` + `src/lib/photos.ts` – Foto-Check (Route `/photos`): Fortschrittsfotos in vier Posen als JPEG-Blob in der Dexie-Tabelle `photos` (DB-Version 2), Verkleinerung auf 1280 px beim Speichern, Termine = Montag der Wochen 1/5/9/13/17 mit Fenster ±3 Tage, eigene Sicherung (nicht im JSON-Export), hartes Löschen. Hauptweg für Aufnahmen ist die Kamera-App des Handys über ein Dateifeld mit `capture` (volle Bildqualität); `src/components/PhotoCamera.tsx` (getUserMedia mit Selbstauslöser) ist nur Nebenweg, weil Chrome auf dem Xiaomi bei Größenwünschen nur Querformat und per ImageCapture nur rohe, flaue Bilder liefert.
