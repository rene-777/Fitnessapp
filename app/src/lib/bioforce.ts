...
// Die Skala an den Schwingarmen der Bio Force zeigt amerikanische Pfund pro Seite (5–125 in 5er-Rasten,
// siehe Umrechnungstabelle der Finnlo-Anleitung). Eingegeben und angezeigt wird der Skalenwert,
...
export function loadText(weightKg: number, loadType: LoadType | string) {
  ...
  return `${kgToLb(weightKg)} lb (≈ ${fmtKg(weightKg)} kg)`
}
